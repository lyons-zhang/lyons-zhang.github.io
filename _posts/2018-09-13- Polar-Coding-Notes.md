...
<center>$$\eqalign{ I(W) &= I(X;Y) &= \sum_{y \in \cal Y}\sum_{x \in \cal X} { {p_{X,Y}(x,y) p_X(x)} \over p_X(x) } \log_2 {p_{X,Y}(x,y) \over p_X(x)} {1 \over p_Y(y)} \\ &= \sum_{y \in \cal Y}\sum_{x \in \cal X} {p_{Y|X}(y|x)p_X(x)} \log_2 {p_{Y|X}(y|x) \over p_Y(y)}}$$</center>  
