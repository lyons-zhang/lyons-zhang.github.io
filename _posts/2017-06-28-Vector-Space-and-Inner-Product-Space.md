...
For each scalar $$\alphaa$$ and each $$\vec v \in \mathcal V$$ there is a unique vector $$\alphaa \vec v \in \mathcal V$$ called the scalar product of $$\alphaa$$ and $$\vec v$$ satisfying:   
* Scalar associativity:	$$\alphaa (\beta \vec v) = (\alpha \beta)\vec v$$ for all scalars $$\alpha,\beta$$, and all $$\vec v \in \mathcal V$$.
